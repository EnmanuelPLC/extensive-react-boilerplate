...
import { useEditCampaignService } from "@/services/api/services/campaigns";
import { useGetCampaignQuery } from "../../queries/queries";
...
const useValidationSchema = () => {
  ...
  const { t } = useTranslation("admin-panel-campaigns-edit");
...
function FormEdit() {
  ...
  const fetchEditCampaign = useEditCampaignService();
  const { t } = useTranslation("admin-panel-campaigns-edit");
  const validationSchema = useValidationSchema();
  const { data: initialData } = useGetCampaignQuery({ id: entityId });
  ...
  const onSubmit = handleSubmit(
    async (
      ...
      const { data, status } = await fetchEditCampaign({
        id: entityId,
        data: {
